...
/**
 * Buffered reader for WebSocket data
 * Accumulates incoming strings and provides line-based access
 */
export class WebSocketBufferedReader {
    private buffer: string = "";
    private linePromises: Array<{
    ...
    /**
     * Processes the buffer to extract complete lines
     */
    private processBuffer(): void {
        while (this.linePromises.length > 0) {
            const lineIndex = this.buffer.indexOf("\n");
            if (lineIndex === -1) {
                break;
            }

            const line = this.buffer.substring(0, lineIndex).trim();
            this.buffer = this.buffer.substring(lineIndex + 1);

            const promise = this.linePromises.shift();
            if (promise) {
     ...
     */
    readLine(timeoutMs: number = 0): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            const promise = {
                ...
                timer: setTimeout(() => {
                    ...
                    const index = this.linePromises.indexOf(promise);
                    if (index !== -1) {
                        this.linePromises.splice(index, 1);
                    }
                    reject(new Error("Timeout waiting for line"));
                }, timeoutMs || 30000)
            };

            this.linePromises.push(promise);
        });
    }

    /**
     * Waits for a line with timeout (returns empty string on timeout)
     */
    async waitForLine(timeoutMs: number = 500): Promise<string> {
        try {
            return await this.readLine(timeoutMs);
        } catch (error) {
            return "";
        }
    }
    ...
    /**
     * Gets the reader function for use with WebSocketAdapter.addReader()
     */
    getReader(): (data: string) => void {
        return (data: string) => this.onData(data);
    }
}
