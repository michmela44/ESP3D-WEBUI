...
export interface ExportStructure {
    [category: string]: {
        [subCategory: string]: ExportEntry[];
    };
}

function filterProperties(rawFeatures: FeaturesStructure): ExportStructure {
    return Object.keys(rawFeatures).reduce((acc, categoryName) => {
        ...
        const subCategories = Object.keys(category).reduce(
            ...
            {} as { [key: string]: ExportEntry[] }
        )
        return { ...acc, [categoryName]: subCategories }
    }, {} as ExportStructure)
}

function exportFeatures(features: FeaturesStructure): void {
