...
import { Loader, Sun, Power, Wind } from "preact-feather"
...
const LaserControls: FunctionalComponent = () => {
    ...
    // Feed rate + laser speed + mode — same as SpindleControls
    const states_array = [
    ...
    const hasAny = states && (
        (states as any).feed_rate ||
        (states as any).spindle_speed ||
        (states as any).spindle_mode
    )
...
const LaserPanel: FunctionalComponent = () => {
    ...
    // Air assist current mode helper
    const airMode = (): string => {
        if (!states) return ""
        const cm = states["coolant_mode"] as StateValue | undefined
        if (!cm) return ""
        return Array.isArray(cm) ? cm.map((i) => i.value).join(" ") : cm.value
    }
                                        ...
                                        <div key={control.id} class="states-buttons-container">
                                            {control.elements.map((element: any) => {
                                                if (element.type === "button") {
                                                    ...
                                                    if (states && element.mode && states[element.mode]) {
                                                    ...
                                                    return (
                                                        <ButtonImg key={element.id}
                                                            ...
                                                            disabled={element.useinput ? hasError() : false}
                                                            icon={element.icon}
                                                            tooltip
                                                            iconRight={element.iconRight}
                                                            data-tooltip={T(element.tooltip)}
                                                            mode={element.mode}
                                                            useinput={element.useinput}
                                                    ...
                                                    const generateValidation = (element: any) => {
                                                        let validation = { message: null, valid: true, modified: false }
                                                        if (element.value.current < element.min || element.value.current.length === 0) {
                                                    ...
                                                    return (
                                                        <Field key={element.id}
                                                            ...
                                                            setValue={(val: any, update = false) => {
                                                                if (!update) element.value.current = val
                                                                const v = generateValidation(element)
                                                                setvalidation(v)
                                                                if (v.valid && element.variableName) {
                                                                    variablesList.addCommand({ name: element.variableName, value: element.value.current })
                                                                }
                                                            }}
                    ...
                    <div class="field-group-content maxwidth">
                        <div class="states-buttons-container">
                            <ButtonImg
                                ...
                                icon={<Wind />}
                                tooltip
                                data-tooltip={T("CN202")}
                                className={`tooltip tooltip-right${airMode() === "M8" ? " btn-primary" : ""}`}
                                onClick={(e: TargetedMouseEvent<HTMLButtonElement>) => {
                                    useUiContextFn.haptic()
                                    e.currentTarget.blur()
                                    targetCommands("M8")
                            ...
                            <ButtonImg
                                ...
                                icon={<Power />}
                                tooltip
                                data-tooltip={T("CN203")}
                                className={`tooltip tooltip-left${airMode() === "M9" ? " btn-primary" : ""}`}
                                onClick={(e: TargetedMouseEvent<HTMLButtonElement>) => {
                                    useUiContextFn.haptic()
                                    e.currentTarget.blur()
                                    targetCommands("M9")
